...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        sentence_cp = list(sentence)
        # Adding start and end into sentence based on window_size
        for i in range(window_size):
            sentence_cp.insert(0, start)
            sentence_cp.append(end)
        # Construct the result
        for i in range(len_sentence):
            windowed_element = []
            for j in range(2 * window_size + 1):
                windowed_element += sentence_cp[i + j]
            windowed_element = (windowed_element, labels[i])
            windowed_data.append(windowed_element)
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        # Implement the 1-hidden-layer NN
    ...
    def add_loss_op(self, pred):
        ...
        loss_total = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,
                                                                    logits=pred)
