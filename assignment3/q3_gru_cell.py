...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            # Reset gate parameters initialization
            ...
            # Update gate parameters initialization
            ...
            # h_tilda gate parameters initialization
            ...
            h_tilda = tf.tanh(tf.matmul(inputs, U_o) + tf.matmul(tf.multiply(r_t, state), W_o) + b_o)
            new_state = tf.multiply(z_t, state) + tf.multiply((1 - z_t), h_tilda)
            ### END YOUR CODE ###
